...
(function initWardrobe() {
    ...
    const SW_TYPES = [
        ...
        { id: 'underwear', label: 'Бельё',           icon: 'fa-heart' },
        { id: 'outer',     label: 'Верхняя',         icon: 'fa-mitten' },
        { id: 'other',     label: 'Другое',          icon: 'fa-tag' },
    ];
    const SW_TYPE_IDS = SW_TYPES.map(t => t.id);
    function swTypeOf(o) { return (o && SW_TYPE_IDS.includes(o.type)) ? o.type : 'other'; }
    function swTypeMeta(id) { return SW_TYPES.find(t => t.id === id) || SW_TYPES[SW_TYPES.length - 1]; }
    ...
    let swFilter = 'all';
    // Пагинация: сколько карточек показываем на одной странице (чтобы не грузить весь гардероб в DOM сразу).
    const SW_PAGE_SIZE = 12;
    let swPage = 0;
    ...
    function swOpenModal() {
        ...
        m.innerHTML = `
            <div class="sw-modal-header">
                ...
                <div class="sw-modal-header-btns">
                    ...
                    <div class="sw-header-btn sw-btn-cleanup" title="Чистка неиспользуемых файлов"><i class="fa-solid fa-broom"></i></div>
        ...
        m.querySelector('.sw-btn-cleanup').addEventListener('click', swOpenCleanup);
    ...
    function swRender() {
        ...
        // ── Кнопки режима: Перс / Общий (для обоих табов — Бот и Юзер) ──
        {
            ...
            const pending = useShared ? swCountPendingMigration(swTab) : 0;
            h += `<div class="sw-mode-row">
                ...
                ${useShared && pending > 0 ? `<button class="sw-import-btn" id="sw-import-old" title="Перенести старые наряды со всех персонажей в общий (копии; оригиналы не удаляются)"><i class="fa-solid fa-cloud-arrow-down"></i> Импорт (${pending})</button>` : ''}
            </div>`;
        }
        ...
        const shown = swFilter === 'all' ? outfits : outfits.filter(o => swTypeOf(o) === swFilter);
        ...
        for (const o of pageItems) {
            ...
            const opts = SW_TYPES.map(t => `<option value="${t.id}" ${swTypeOf(o) === t.id ? 'selected' : ''}>${esc(t.label)}</option>`).join('');
        ...
        // Импорт старых нарядов в общий (для текущей стороны — Бот или Юзер)
        document.getElementById('sw-import-old')?.addEventListener('click', async (e) => {
            const btn = e.currentTarget;
            if (!confirm('Перенести все старые наряды в общий гардероб?\n\nЭто КОПИИ — оригиналы у персонажей останутся.')) return;
            btn.disabled = true; btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Импорт…';
            try { await swMigrateToShared(v.side); } catch (x) { swLog('WARN', 'migrate failed:', x.message); }
        ...
        for (const b of c.querySelectorAll('.sw-mode-btn')) {
            b.addEventListener('click', async () => {
                ...
                // При переходе в общий — предложить перенести старые наряды этой стороны.
                if (wantShared) {
                    const pending = swCountPendingMigration(swTab);
                    if (pending > 0 && confirm(`Найдено ${pending} старых нарядов. Перенести их в общий гардероб?\n\nОригиналы у персонажей НЕ удаляются.`)) {
                        await swMigrateToShared(swTab); // сам наденет то, что было одето у текущего персонажа
                    } else {
    ...
    function swOpenOutfitForm({ mode, view, base64 = null, item = null, defaultName = '' }) {
        ...
        const curType = isEdit ? swTypeOf(item) : (SW_TYPE_IDS.includes(swFilter) ? swFilter : 'other');
        ...
        panel.innerHTML = `
            ...
            <div class="sw-form-body">
                ...
                <select class="text_pole sw-form-input" id="sw-form-type">${SW_TYPES.map(t => `<option value="${t.id}" ${curType === t.id ? 'selected' : ''}>${esc(t.label)}</option>`).join('')}</select>
        ...
        panel.querySelector('.sw-form-save').addEventListener('click', async () => {
            ...
            try {
                if (isEdit) {
                    ...
                    view.add(newItem);
                    if (view.shared) swPreloadSharedActive(view.side);
                    swPage = Number.MAX_SAFE_INTEGER; // показать последнюю страницу с новым образом
                }
                close();
                swRender(); swUpdatePromptInjection(); swInjectFloatingBtn();
    ...
    function swOpenCleanup() {
        document.getElementById('sw-cleanup-overlay')?.remove();
        const ov = document.createElement('div'); ov.id = 'sw-cleanup-overlay';
        ov.addEventListener('click', (e) => { if (e.target === ov) ov.remove(); });
        const panel = document.createElement('div'); panel.id = 'sw-cleanup-panel';
        panel.innerHTML = `
            <div class="sw-cleanup-header"><span><i class="fa-solid fa-broom"></i> Чистка файлов</span><div class="sw-cleanup-close" title="Закрыть"><i class="fa-solid fa-xmark"></i></div></div>
            <div class="sw-cleanup-body" id="sw-cleanup-body"><div class="sw-cleanup-loading"><i class="fa-solid fa-spinner fa-spin"></i> Сканирование…</div></div>`;
        ov.appendChild(panel); document.body.appendChild(ov);
        panel.querySelector('.sw-cleanup-close').addEventListener('click', () => ov.remove());

        const selected = new Set();
        let state = null;
        const body = panel.querySelector('#sw-cleanup-body');
    ...
    window.swOpenCleanup = swOpenCleanup;
